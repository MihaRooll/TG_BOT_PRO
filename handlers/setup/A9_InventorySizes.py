...
def open_template_numbers(chat_id: int, mk: str):
    ...
    if WIZ[chat_id].get("flow_origin") == "step3":
        kb.add(types.InlineKeyboardButton("↩️ К Шагу 3/4 — Макеты", callback_data="setup:tmpl_back"))
    if scope:
        header = "📦 Остатки → Макеты — Область: Глобально\nВыберите номера макетов."
    else:
        merch_name = WIZ[chat_id]['data']['merch'][mk]['name_ru']
        header = f"📦 Остатки → Макеты — Область: {merch_name}\nВыберите номера макетов."
    edit(chat_id, header, kb)
...
def open_template_qty_spinner(chat_id: int, mk: str, num: str):
    ...
    if WIZ[chat_id].get("flow_origin") == "step3":
        kb.add(types.InlineKeyboardButton("↩️ К Шагу 3/4 — Макеты", callback_data="setup:tmpl_back"))
    if WIZ[chat_id]["data"].get("_inv_tmpl_scope"):
        header = f"📦 Остатки → Макеты — Область: Глобально\nВведите количество макетов <b>{num}</b>:\nТекущее: <b>{cur}</b>"
    else:
        merch_name = WIZ[chat_id]['data']['merch'][mk]['name_ru']
        header = f"📦 Остатки → Макеты — Область: {merch_name}\nВведите количество макетов <b>{num}</b>:\nТекущее: <b>{cur}</b>"
    edit(chat_id, header, kb)
...
def apply_all_templates(chat_id: int, mk: str):
    ...
    if WIZ[chat_id].get("flow_origin") == "step3":
        kb.add(types.InlineKeyboardButton("↩️ К Шагу 3/4 — Макеты", callback_data="setup:tmpl_back"))
    if WIZ[chat_id]["data"].get("_inv_tmpl_scope"):
        header = "📦 Остатки → Макеты — Область: Глобально\nВведите одно число ко всем макетам."
    else:
        merch_name = WIZ[chat_id]['data']['merch'][mk]['name_ru']
        header = f"📦 Остатки → Макеты — Область: {merch_name}\nВведите одно число ко всем макетам."
    edit(chat_id, header, kb)

...
def render_letters_hub(chat_id: int):
    ...
    alphabet_line = (
        ("Латиница" if rules.get('allow_latin') else "") +
        (" / " if rules.get('allow_latin') and rules.get('allow_cyrillic') else "") +
        ("Кириллица" if rules.get('allow_cyrillic') else "")
    ) or "—"
    space_line = "Разрешен ✔️" if rules.get('allow_space') else "Запрещен ✖️"

    text = (
        "<pre><b>🔤 ШАГ 2/4: НАСТРОЙКА БУКВ И ЦИФР 🔢</b>\\n\\n"
        "<b>✨ Буквы</b>\\n"
    ...
    kb.add(types.InlineKeyboardButton(f"Цифры: {'вкл' if feats.get('numbers') else 'выкл'}", callback_data="setup:feature_toggle:numbers"))
    ...
    kb.add(types.InlineKeyboardButton(f"Кириллица: {'да' if rules.get('allow_cyrillic') else 'нет'}", callback_data="setup:rule_toggle:allow_cyrillic"))
    ...
    kb.add(types.InlineKeyboardButton("Пределы ✏️", callback_data="setup:limits"))
...
def render_limits_progress(chat_id: int):
    ...
    text = (
        ...
        "Выберите этап или введите значения по порядку."
    )
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(types.InlineKeyboardButton("1) Ввести длину текста", callback_data="setup:limits_edit:text_len"))
    kb.add(types.InlineKeyboardButton("2) Ввести макс. номер", callback_data="setup:limits_edit:max_num"))

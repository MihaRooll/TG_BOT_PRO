...
def home_text(d: dict) -> str:
    ...
    block.append(f"<b>1. 🛍 Мерч</b> [{_on_off(merch_on)}]")
    block.append(f"• <b>Цвета:</b> {'✅' if colors_ok else '❌'}")
    block.append(f"• <b>Размеры:</b> {'✅' if sizes_ok  else '❌'}")
    block.append("")

    block.append(f"<b>2. 🔤 Буквы</b> [{_on_off(feats.get('letters', False))}]")
    alph = []
    ...
    block.append(f"<b>3. 🔢 Цифры</b> [{_on_off(feats.get('numbers', False))}]")
    block.append("• <b>Соответствия:</b>")
    block.append(f"&nbsp;&nbsp;Мерч/Цвет → Цвет текста {'✅' if mapping_ok else '❌'}")
    block.append("")

    block.append(f"<b>4. 🖼 Макеты</b> [{_on_off(nums_set)}]")
    block.append(f"• <b>Номера:</b> {'✅' if nums_set else '❌'}")
    block.append(f"• <b>Коллажи:</b> {coll_count} {'🟢' if coll_count else '🚫'}")
    block.append("")

    block.append(f"<b>5. 📦 Остатки</b> [{_on_off(bool(inv_merch))}]")

...
def render_types(chat_id: int):
    ...
    edit(chat_id, "Шаг 1/4. Выберите виды мерча (кнопки‑переключатели).", kb)
...
def ask_custom(chat_id: int):
    ...
    edit(chat_id, "Введите <b>название</b> нового мерча (ключ создадим автоматически).", kb)
...
def render_colors(chat_id: int, mk: str):
    ...
    edit(chat_id, f"Шаг 1.1/4. <b>{item['name_ru']}</b> — выберите цвета.", kb)
...
def ask_custom_color(chat_id: int, mk: str):
    ...
    edit(chat_id, "Введите <b>название цвета</b> (ключ создадим автоматически).", kb)
...
def render_sizes(chat_id: int, mk: str):
    ...
    edit(chat_id, f"Шаг 1.2/4. <b>{item['name_ru']}</b> — размеры.\\nТекущие: {sizes_text}", kb)
...
def ask_custom_sizes(chat_id: int, mk: str):
    ...
    edit(chat_id, "Введите размеры через запятую (пример: 3XS,2XS,XS,S,M,L,XL).", kb)

...
def setup_router(c: types.CallbackQuery):
    ...
    if cmd == "tmpl_nums":
        ...
        kb.add(types.InlineKeyboardButton("Линтер областей", callback_data="setup:tmpl_scope_lint"))
        kb.add(types.InlineKeyboardButton("⬅️ Назад", callback_data="setup:tmpls"))
    ...
    if cmd == "tmpl_scope_lint":
        from .A6_TemplatesNumbers import scope_lint
        scope_lint(chat_id); return
    if cmd == "tmpl_scope_lint_move":
        from .A6_TemplatesNumbers import apply_lint_move
        apply_lint_move(chat_id); return
    ...
    if cmd == "tmpl_collages_global":
        TCOLL.start_global(chat_id, done_cb=render_templates_home); return
...
def _during_setup(m: types.Message):
    ...
    # --- коллажи (фото) ---
        ...
        tag = "Глобально" if mk == "__global__" else WIZ[chat_id]["data"].get("merch", {}).get(mk, {}).get("name_ru", mk)
        edit(chat_id,

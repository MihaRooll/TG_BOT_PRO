...
def order_confirm_yes(c: types.CallbackQuery):
    ...
    final_text = (
        f"🚀🛒 <a href=\"tg://user?id={chat_id}\">ВАШ ЗАКАЗ #{order_no}</a>! 🤖 "
        f"<a href=\"https://t.me/{username}\">оформлен</a> 🌟\n"
        f"🛍 {html.escape(merch_name)} | Цвет: {html.escape(color_name)} | {html.escape(d['size'])}\n"
        f"📝 {html.escape(text_val)} | {html.escape(number_val)} | Цвет: {html.escape(text_color)}\n"
        f"🖼 Макеты: {html.escape(tpl_disp)}\n"
        f"📞 {html.escape(phone)}\n"
